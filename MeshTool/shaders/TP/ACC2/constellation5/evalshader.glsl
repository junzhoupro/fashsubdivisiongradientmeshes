...
vec2 getQuadCoords() {
  ...
  if (instance == 0)
    return vec2(.5 * gl_TessCoord.x, .5 * gl_TessCoord.y);
  if (instance == 1)
    return vec2(.5 + .5 * gl_TessCoord.x, .5 * gl_TessCoord.y);
  if (instance == 2)
    return vec2(.5 * gl_TessCoord.x, .5 + .5 * gl_TessCoord.y);
  if (instance == 3)
    return vec2(.5 + .5 * gl_TessCoord.x, .5 + .5 * gl_TessCoord.y);
}
